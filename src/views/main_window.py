...
from PySide6.QtWidgets import QLabel, QMainWindow, QVBoxLayout, QWidget, QTableView, QMessageBox
...
from src.core.config import PROFESSIONAL_COLUMNS
...
class MainWindow(QMainWindow):
    ...
    def load_market_data(self):
        ...
        try:
            # Идем в сеть за сырым словарем (JSON) через сетевой клиент
            raw_data = self.moex_client.get_clean_data()  # Вызови твой метод получения данных
            
            # Передаем сырой JSON в твой динамический пар
            df = self.moex_parser.parse_to_dataframe(raw_data)
            
            if df.empty:
                self.statusBar().showMessage("Предупреждение: получен пустой массив данных.")
                logger.warning("Парсер вернул пустой DataFrame после обработки.")
                return
                
            # Шаг 3: Передаем очищенный и типизированный DataFrame в модель Qt (День 7)
            self.table_model.set_dataframe(df)
            
            self.statusBar().showMessage(f"Данные успешно обновлены. Всего бумаг в кэше: {len(df)}")
            ...
            logger.error(f"Непредвиденная ошибка при обновлении интерфейса: {e}", exc_info=True)
            QMessageBox.critical(self, "Критическая ошибка", f"Внутренний сбой системы:\n{e}")
